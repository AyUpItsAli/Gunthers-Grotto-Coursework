...
func get_player_tile_pos() -> Vector2:
	...
	if not is_solid_tile(actual_tile_pos):
		...
		if not is_solid_tile(next_tile_pos):
...
func update_selected_tile():
	...
	var mouse_tile_pos = world_to_map(mouse_pos)
	...
	var solid = is_solid_tile(tile_pos)
	...
	# If the tile is not out of bounds, is solid, and the distance from
	# the player to the closest edge or vertex is less than the maximum
	# distance, the tile can be selected from...
	if not out_of_bounds and solid and distance <= MAX_DISTANCE:
		var nbr_tile_1 = Vector2(tile_pos.x - direction.x, tile_pos.y)
		var nbr_tile_2 = Vector2(tile_pos.x, tile_pos.y - direction.y)
		# If the tile is blocked from the player by its neighbouring tiles...
		if is_solid_tile(nbr_tile_1) and is_solid_tile(nbr_tile_2):
			offset = Vector2.ONE * cell_size / 2
			var nbr_centre_1 = map_to_world(nbr_tile_1) + offset
			var nbr_centre_2 = map_to_world(nbr_tile_2) + offset
			# ...select the neighbour tile closest to the mouse position.
			if mouse_pos.distance_to(nbr_centre_1) < mouse_pos.distance_to(nbr_centre_2):
				select_tile(nbr_tile_1)
			else:
				select_tile(nbr_tile_2)
		else: # ... otherwise, select the original tile
